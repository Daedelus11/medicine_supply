/* ============================================================
   contacts-core.js - FLS Client Portal - Shared Core v1.1
   Project contacts: D3 relationship map + card directory.
   Requires: FLS_CONFIG + D3 v7 loaded before this script.
   Schema: supports both old (name/organization/firm/title)
           and new (firstname/lastname/role/org/position) formats.
   ============================================================ */
(function () {
  'use strict';
  const C = FLS_CONFIG;
  ...
  const ORG_COL = {Owner:'#BA7517',Architecture:'#FEC422',Contractor:'#1E6BA8',
    Engineer:'#2D7A3A',Engineering:'#2D7A3A',Consultant:'#8C3562',Other:'#7D6B57',AHJ:'#555550'};
  ...
  function renderPage() {
    ...
    const ns = {
      ...
      con:`<svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>`,
      doc:`<svg viewBox="0 0 24 24"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>`,
      lck:`<svg viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>`,
      ca:`<svg viewBox="0 0 24 24"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>`,
    };
  ...
  function csvRowToContact(row, idx) {
    // Support old schema (name, organization, firm, title) and new schema (firstname, lastname, role, org, position)
  ...
  function buildMindMap(rows) {
    ...
    const mm=rows.filter(r=>(r.organization||r.role)&&(r.firm||r.org)).map(r=>({name:r.name||(((r.firstname||'')+' '+(r.lastname||'')).trim()),org:r.organization||r.role||'',firm:r.firm||r.org||'',title:r.title||r.position||'',hierarchy:r.hierarchy||'',address:r.address||'',phone:r.phone||'',email:r.email||'',img:r['contactimage']||r['contact image']||r['photourl']||r['photo']||r['image']||'',color:r.color||r['nodecolor']||r['node color']||''}));
    const FM=new Map();
    ...
    FM.forEach(firm=>{const pc=firm.people.find(p=>isHex(p.color));firm.nodeColor=pc?pc.color.trim():(ORG_COL[firm.org]||'#555550');});
    ...
    hier.descendants().forEach(d=>{
      ...
      const col=d.data.nodeColor||(ORG_COL[d.data.org]||'#555550');
    ...
    lgEl.innerHTML=orgs.map(o=>{const nc=mainF.find(f=>f.org===o)?.nodeColor||(ORG_COL[o]||'#555550');return`<div class="mm-li"><div class="mm-ld" style="background:${nc}"></div>${o}</div>`;}).join('')+(ahjF.length?`<div class="mm-li"><div class="mm-ld" style="background:#555550"></div>AHJ</div>`:'');
