/* ============================================================
   contacts-core.js - FLS Client Portal - Shared Core v1.3
   Halo: Owner hub + Fine Line Studio only (by name, not org).
   Sizes: Owner r=46, FLS r=30, depth1 r=16, depth2 r=13.
   Colors: sheet Code column only, gray fallback.
   ============================================================ */
(function () {
  'use strict';
  const C = FLS_CONFIG;
  ...
  function buildMindMap(rows) {
    ...
    const flsN=hier.descendants().find(d=>d.data.name==='Fine Line Studio');
    ...
    hier.descendants().forEach(d=>{
      const isHub=d.data.isHub,isFLS=d.data.name==='Fine Line Studio';
      const r=isHub?46:isFLS?30:d.depth===1?16:d.depth===2?13:10;
